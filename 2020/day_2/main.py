...
def count_valid_passwords_first(list_of_passwords):
    """
    Count the number of passwords that have a
    given password policy numbers of letters.
    """
    result = [1 for line
              in list_of_passwords
              if line["password"].count(line["letter"])
              in range(*line["password_policy"])]
    return sum(result)


def count_valid_passwords_second(list_of_passwords):
    """
    Count the number of passwords that have
    only one letter in indices of password policy.
    """
    result = [1 for line
              in list_of_passwords
              if (line["password"][line["password_policy"][0]-1] == line["letter"]) ^
